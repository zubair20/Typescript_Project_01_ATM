...
const userIdValidator = (userId: string) => userId.length < 3 ? "User ID should be atleast 3 Characters" : true
...
const transactionTypeQuestions = [
  {
    ...
    choices: [
      transactionType.CASH_WITHDRAWL,
...
function askCredentials(){
  ...
  .then((answers: User) => {
    if(answers.userId !== 'zubair20' && answers.userPin !==  '1234'){
...
function askTransactionType(){
  ...
  .then((answers) => {
    if(answers.transactionType === transactionType.CASH_WITHDRAWL){
      transaction(transactionType.CASH_WITHDRAWL)
...
function transaction(type: transactionType.CASH_WITHDRAWL | transactionType.CASH_DEPOSIT){
  inquirer
  .prompt([
    {
      type: 'number',
  ...
  .then((answers) => {
    ...
    if(type === transactionType.CASH_WITHDRAWL){
